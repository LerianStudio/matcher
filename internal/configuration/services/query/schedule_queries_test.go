...
func TestListSchedules_NilRepository(t *testing.T) {
	t.Parallel()
	...
	_, err = uc.ListSchedules(context.Background(), uuid.New())
...
func TestListSchedules_Success(t *testing.T) {
	...
	expected := []*entities.ReconciliationSchedule{
		{ID: uuid.New(), ContextID: contextID, CronExpression: "0 0 * * *", Enabled: true},
		{ID: uuid.New(), ContextID: contextID, CronExpression: "0 6 * * 1", Enabled: false},
	}

	scheduleRepo := &mockScheduleRepo{
		findByContextIDFn: func(_ context.Context, id uuid.UUID) ([]*entities.ReconciliationSchedule, error) {
	...
	result, err := uc.ListSchedules(context.Background(), contextID)
...
func TestListSchedules_EmptyResult(t *testing.T) {
	t.Parallel()
	...
	result, err := uc.ListSchedules(context.Background(), uuid.New())

	require.NoError(t, err)
	require.NotNil(t, result, "nil result should be converted to empty slice")
