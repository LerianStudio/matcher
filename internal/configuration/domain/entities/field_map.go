...
var ErrFieldMapNil = errors.New("field map is nil")

// ErrFieldMapContextRequired is returned when the context_id is not provided.
var ErrFieldMapContextRequired = errors.New("context_id is required")

// ErrFieldMapSourceRequired is returned when the source_id is not provided.
var ErrFieldMapSourceRequired = errors.New("source_id is required")

// ErrFieldMapMappingRequired is returned when the mapping is not provided.
var ErrFieldMapMappingRequired = errors.New("mapping is required")

// ErrFieldMapMappingValueEmpty is returned when a mapping value is an empty string.
var ErrFieldMapMappingValueEmpty = errors.New("mapping values must be non-empty strings")
...
func NewFieldMap(
	...
	if err := validateMappingValues(input.Mapping); err != nil {
		return nil, err
	}
...
// validateMappingValues checks that all string-typed mapping values are non-empty.
// Non-string values (nested objects, numbers, booleans) are allowed through without
// additional checks, as the ingestion context is responsible for detailed validation.
func validateMappingValues(mapping map[string]any) error {
