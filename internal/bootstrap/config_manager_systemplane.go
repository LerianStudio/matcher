...
func configFromSnapshot(snap domain.Snapshot) *Config {
	...
	cfg.Fetcher.MaxExtractionBytes = snapInt64(snap, "fetcher.max_extraction_bytes", defaultFetcherMaxExtractionBytes)
	cfg.Fetcher.BridgeIntervalSec = snapInt(snap, "fetcher.bridge_interval_sec", defaultBridgeIntervalSec)
	cfg.Fetcher.BridgeBatchSize = snapInt(snap, "fetcher.bridge_batch_size", defaultBridgeBatchSize)
	cfg.Fetcher.BridgeStaleThresholdSec = snapInt(snap, "fetcher.bridge_stale_threshold_sec", defaultBridgeStaleThresholdSec)
	cfg.Fetcher.BridgeRetryMaxAttempts = snapInt(snap, "fetcher.bridge_retry_max_attempts", defaultBridgeRetryMaxAttempts)
	cfg.Fetcher.CustodyRetentionSweepIntervalSec = snapInt(snap, "fetcher.custody_retention_sweep_interval_sec", defaultCustodyRetentionSweepIntervalSec)
	cfg.Fetcher.CustodyRetentionGracePeriodSec = snapInt(snap, "fetcher.custody_retention_grace_period_sec", defaultCustodyRetentionGracePeriodSec)
