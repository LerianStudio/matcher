...
func TestUseCase_CountMatched_Success(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountMatched(gomock.Any(), filter).Return(int64(42), nil)

	uc, err := NewUseCase(mockRepo)
	require.NoError(t, err)

	count, err := uc.CountMatched(ctx, filter)

	require.NoError(t, err)
	assert.Equal(t, int64(42), count)
}

func TestUseCase_CountMatched_Error(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountMatched(gomock.Any(), filter).Return(int64(0), errTestRepoError)
	...
	assert.Contains(t, err.Error(), "counting matched")
}

func TestUseCase_CountUnmatched_Success(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountUnmatched(gomock.Any(), filter).Return(int64(15), nil)

	uc, err := NewUseCase(mockRepo)
	require.NoError(t, err)

	count, err := uc.CountUnmatched(ctx, filter)

	require.NoError(t, err)
	assert.Equal(t, int64(15), count)
}

func TestUseCase_CountUnmatched_Error(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountUnmatched(gomock.Any(), filter).Return(int64(0), errTestRepoError)
	...
	require.Error(t, err)
	assert.Equal(t, int64(0), count)
	assert.Contains(t, err.Error(), "counting unmatched")
}

func TestUseCase_CountTransactions_Success(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountTransactions(gomock.Any(), filter).Return(int64(999), nil)

	uc, err := NewUseCase(mockRepo)
	require.NoError(t, err)

	count, err := uc.CountTransactions(ctx, filter)

	require.NoError(t, err)
	assert.Equal(t, int64(999), count)
}

func TestUseCase_CountTransactions_Error(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountTransactions(gomock.Any(), filter).Return(int64(0), errTestRepoError)
	...
	require.Error(t, err)
	assert.Equal(t, int64(0), count)
	assert.Contains(t, err.Error(), "counting transactions")
}

func TestUseCase_CountExceptions_Success(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountExceptions(gomock.Any(), filter).Return(int64(7), nil)

	uc, err := NewUseCase(mockRepo)
	require.NoError(t, err)

	count, err := uc.CountExceptions(ctx, filter)

	require.NoError(t, err)
	assert.Equal(t, int64(7), count)
}

func TestUseCase_CountExceptions_Error(t *testing.T) {
	t.Parallel()
	...
	mockRepo.EXPECT().CountExceptions(gomock.Any(), filter).Return(int64(0), errTestRepoError)
	...
	require.Error(t, err)
	assert.Equal(t, int64(0), count)
	assert.Contains(t, err.Error(), "counting exceptions")
}
