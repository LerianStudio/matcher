...
type varianceFeeVariance struct {
	SourceID      uuid.UUID
	Currency      string
	StructureType string // FLAT, PERCENTAGE, TIERED
	Expected      decimal.Decimal
	Actual        decimal.Decimal
	Delta         decimal.Decimal
}

// varianceRateKey uniquely identifies a rate by (structure_type, currency).
type varianceRateKey struct {
...
type varianceSeedContext struct {
	rateIDs map[varianceRateKey]uuid.UUID
	jobIDs  map[uuid.UUID]uuid.UUID
	runID   uuid.UUID
	ruleID  uuid.UUID
}

// insertVarianceData inserts the full FK chain (rates → ingestion_jobs → transactions →
...
// variance row. A shared rate per (StructureType, Currency) pair is auto-created.
//
...
// insertVarianceParents creates rates, ingestion jobs, and a match run—the
...
func insertVarianceParents(
	...
	sc := varianceSeedContext{
		rateIDs: make(map[varianceRateKey]uuid.UUID),
		jobIDs:  make(map[uuid.UUID]uuid.UUID),
		ruleID:  seed.RuleID,
	}

	// Deduplicate rates by (structure_type, currency).
	for _, variance := range variances {
		key := varianceRateKey{structureType: variance.StructureType, currency: variance.Currency}
		if _, exists := sc.rateIDs[key]; exists {
			continue
		}

		rateID := uuid.New()
		sc.rateIDs[key] = rateID

		_, err := tx.ExecContext(ctx, `
			INSERT INTO rates (id, tenant_id, name, currency, structure_type, structure, created_at, updated_at)
			VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		`, rateID, seed.TenantID,
			fmt.Sprintf("Rate %s %s %s", variance.StructureType, variance.Currency, rateID.String()[:8]),
			variance.Currency, variance.StructureType, `{"amount":"1.00"}`, createdAt, createdAt)
		if err != nil {
			return sc, fmt.Errorf("insert rate: %w", err)
		}
	}
...
func insertSingleVariance(
	...
	rKey := varianceRateKey{structureType: feeVar.StructureType, currency: feeVar.Currency}

	_, err = tx.ExecContext(ctx, `
		INSERT INTO match_fee_variances
			(id, context_id, run_id, match_group_id, transaction_id, rate_id,
	...
	`, uuid.New(), seed.ContextID, sc.runID, groupID, txID, sc.rateIDs[rKey],
...
// same (source_id, currency, structure_type) and verifies that GetVarianceReport
...
func TestVarianceReport_SingleGroupAggregation(t *testing.T) {
	...
	integration.RunWithDatabase(t, func(t *testing.T, h *integration.TestHarness) {
		...
		// 3 variances: same source, same currency, same structure type.
		variances := []varianceFeeVariance{
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(10.00), Actual: decimal.NewFromFloat(8.50), Delta: decimal.NewFromFloat(-1.50)},
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(20.00), Actual: decimal.NewFromFloat(22.00), Delta: decimal.NewFromFloat(2.00)},
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(15.00), Actual: decimal.NewFromFloat(15.00), Delta: decimal.NewFromFloat(0.00)},
		}
		...
		require.Equal(t, "FLAT", row.FeeType)
...
func TestVarianceReport_MultipleGroups(t *testing.T) {
	...
	integration.RunWithDatabase(t, func(t *testing.T, h *integration.TestHarness) {
		...
		variances := []varianceFeeVariance{
			// Source1 + USD (2 rows)
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(100.00), Actual: decimal.NewFromFloat(90.00), Delta: decimal.NewFromFloat(-10.00)},
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(50.00), Actual: decimal.NewFromFloat(55.00), Delta: decimal.NewFromFloat(5.00)},
			// Source1 + EUR (2 rows)
			{SourceID: seed.SourceID, Currency: "EUR", StructureType: "FLAT", Expected: decimal.NewFromFloat(200.00), Actual: decimal.NewFromFloat(200.00), Delta: decimal.NewFromFloat(0.00)},
			{SourceID: seed.SourceID, Currency: "EUR", StructureType: "FLAT", Expected: decimal.NewFromFloat(30.00), Actual: decimal.NewFromFloat(28.00), Delta: decimal.NewFromFloat(-2.00)},
			// Source2 + USD (2 rows)
			{SourceID: source2ID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(75.00), Actual: decimal.NewFromFloat(80.00), Delta: decimal.NewFromFloat(5.00)},
			{SourceID: source2ID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(25.00), Actual: decimal.NewFromFloat(20.00), Delta: decimal.NewFromFloat(-5.00)},
			// Source2 + EUR (2 rows)
			{SourceID: source2ID, Currency: "EUR", StructureType: "FLAT", Expected: decimal.NewFromFloat(60.00), Actual: decimal.NewFromFloat(65.00), Delta: decimal.NewFromFloat(5.00)},
			{SourceID: source2ID, Currency: "EUR", StructureType: "FLAT", Expected: decimal.NewFromFloat(40.00), Actual: decimal.NewFromFloat(38.00), Delta: decimal.NewFromFloat(-2.00)},
		}
...
// (source_id, currency, structure_type) groups and pages through them with
...
func TestVarianceReport_ListVariancePage_Pagination(t *testing.T) {
	...
	integration.RunWithDatabase(t, func(t *testing.T, h *integration.TestHarness) {
		...
		// 5 distinct groups: different combinations of (source, currency, structure_type).
		variances := []varianceFeeVariance{
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(10.00), Actual: decimal.NewFromFloat(9.00), Delta: decimal.NewFromFloat(-1.00)},
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "PERCENTAGE", Expected: decimal.NewFromFloat(20.00), Actual: decimal.NewFromFloat(18.00), Delta: decimal.NewFromFloat(-2.00)},
			{SourceID: seed.SourceID, Currency: "EUR", StructureType: "FLAT", Expected: decimal.NewFromFloat(30.00), Actual: decimal.NewFromFloat(32.00), Delta: decimal.NewFromFloat(2.00)},
			{SourceID: source2ID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(40.00), Actual: decimal.NewFromFloat(40.00), Delta: decimal.NewFromFloat(0.00)},
			{SourceID: source2ID, Currency: "EUR", StructureType: "TIERED", Expected: decimal.NewFromFloat(50.00), Actual: decimal.NewFromFloat(55.00), Delta: decimal.NewFromFloat(5.00)},
		}
		...
		for {
			...
			// Validate cursor format: "uuid:CCC:TYPE".
			require.Regexp(t,
				`^[0-9a-f-]{36}:[A-Z]{3}:[A-Z]+$`,
				nextKey,
				"composite cursor should be source_id:currency:fee_type",
			)
		...
		// Verify ordering: rows should be sorted by (source_id, currency, fee_type) ASC.
		...
		require.True(t, sort.StringsAreSorted(keys),
			"rows across pages should maintain (source_id, currency, fee_type) ASC order")
	})
}
...
func TestVarianceReport_SourceFilter(t *testing.T) {
	...
	integration.RunWithDatabase(t, func(t *testing.T, h *integration.TestHarness) {
		...
		variances := []varianceFeeVariance{
			...
			{SourceID: seed.SourceID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(100.00), Actual: decimal.NewFromFloat(95.00), Delta: decimal.NewFromFloat(-5.00)},
			{SourceID: seed.SourceID, Currency: "EUR", StructureType: "PERCENTAGE", Expected: decimal.NewFromFloat(200.00), Actual: decimal.NewFromFloat(210.00), Delta: decimal.NewFromFloat(10.00)},
			// Source 2: 2 variances.
			{SourceID: source2ID, Currency: "USD", StructureType: "FLAT", Expected: decimal.NewFromFloat(300.00), Actual: decimal.NewFromFloat(280.00), Delta: decimal.NewFromFloat(-20.00)},
			{SourceID: source2ID, Currency: "GBP", StructureType: "TIERED", Expected: decimal.NewFromFloat(400.00), Actual: decimal.NewFromFloat(410.00), Delta: decimal.NewFromFloat(10.00)},
		}
		...
		rows, _, err := repo.GetVarianceReport(ctx, filter)
		require.NoError(t, err)
		require.Len(t, rows, 2, "filtering by source1 should return 2 groups (USD FLAT + EUR PERCENTAGE)")
		...
		byGroup := make(map[groupKey]*entities.VarianceReportRow)
		for _, row := range rows {
			byGroup[groupKey{currency: row.Currency, feeType: row.FeeType}] = row
		}

		usdRow := byGroup[groupKey{currency: "USD", feeType: "FLAT"}]
		require.NotNil(t, usdRow, "USD FLAT group should be returned")
		...
		eurRow := byGroup[groupKey{currency: "EUR", feeType: "PERCENTAGE"}]
		require.NotNil(t, eurRow, "EUR PERCENTAGE group should be returned")
		...
		require.NoError(t, err)
		require.Len(t, rows2, 2, "filtering by source2 should return 2 groups (USD FLAT + GBP TIERED)")
